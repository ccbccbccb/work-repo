...
// 조건문 : switch
// 사용법 : 조건변수와 값을 비교해서 가각의 case 따라 진행됨
//         switch(조건변수) {
    //         case 값:
    //          실행문;
    //           break;
...
// TODO: 3항 연산자(조건문) : *****(React 에서 시도때도 없이 사용함)
// 활용 : 자바, 자바스크립트에서 간단한 조건 문제일 경우 사용함(주로 1줄 표현)
// 사용법 : let 변수명 = (조건식)? 실행문1 : 실행문2;
//    조건식을 판단하여 참이면 실행문1 실행되고, 거짓이면 실행문2가 실행됨
// 예제) 입력값을 받아서 값이 admin 이면 "있음" 이라고 출력하고, 아니면 "없음" 출력하세요
// 조건식 : 3항 연산자 사용
// str이 admin 이면 있음 이라는 문자열이 result 변수에 저장됨

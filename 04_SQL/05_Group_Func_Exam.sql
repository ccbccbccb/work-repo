...
-- 단, 관리자를(MANAGER) 알 수 없는 사원 및 최저 급여가
-- 2000미만인 그룹은 제외시키고, 급여에 대한 내림차순으로 정렬해서 출력하세요
SELECT JOB, MIN(SALARY)
FROM EMPLOYEE
WHERE MANAGER IS NOT NULL
GROUP BY JOB
HAVING MIN(SALARY) >= 2000
ORDER BY MIN(SALARY) DESC; // 내림차순 정렬
...
SELECT DNO, COUNT(DNO), TO_CHAR(HIREDATE, 'YYYY')
       ,COUNT(*) AS CNT
       FROM EMPLOYEE

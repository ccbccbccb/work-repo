...
-- 예제 2) 10번 부서의 소속사원을 출력하세요(전체출력: *)
-- 사원 테이블 : EMPLOYEE
-- 부서번호 컬럼 : DNO
-- 조건절 : WHERE DNO = 10;
SELECT * FROM EMPLOYEE WHERE DNO = 10;

-- 예제 3) 사원명(ENAME) 이 SCOTT 인 사원을 출력하세요(전체출력 *)
-- 힌트) SQL 문자열 : '문자열' (홑따옴표)
...
-- 연습 1) 급여가(SALARY) 1000 과 1500 사이의 사원 조회하기
SELECT * FROM EMPLOYEE WHERE SALARY >= 1000 AND SALARY <= 1500;

-- 예제 6) 부서번호가 10이거나(OR) 직급이 'MANAGER' 인 사원만 출력하기
...
-- 연습 3) 커미션이 300 이거나 500 이거나 1400인 사원 출력하기
...
-- BETWEEN 사용법) SELECT 컬러명 FROM 테이블명 BETWEEN 작은값 AND 큰값;
-- 사용처 : 작은값 ~ 큰값 사이의 결과 사이의 결과 조회하기 할때 사용
...
-- 연습 3) 1982년에 입사한(HIREDATE) 사원에 조회하기
...
-- 4) IN 예약어
-- 예제 10) 상여금(COMMISSION) 300 이거나 500이거나 1400인 사원 조회하기

SELECT * FROM EMPLOYEE WHERE COMMISSION = 300 OR COMMISSION = 500 OR COMMISSION = 1400;

-- 사용법) SELECT 컬럼명 FROM 테이블명 WHERE 컬럼명 IN (값, 값2, ...);
-- 사용처 : OR 로 연결된 데이터를 간단하게 사용 가능
SELECT * FROM EMPLOYEE WHERE COMMISSION (300, 500, 1400);

-- NOT IN 사용
-- 예제 11) 상여금(COMMISSION) 300이 아니고, 500이 아니고, 1400 아닌 사원 조회하기

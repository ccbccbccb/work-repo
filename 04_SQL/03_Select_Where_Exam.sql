...
-- 사원테이블 : EMPLOYEE
SELECT ENAME, SALARY, SALARY*12+100 AS "연간 총 수입" FROM EMPLOYEE ORDER BY SALARY*12+100 DESC;
...
SELECT ENO, ENAME, JOB, SALARY, DNO FROM EMPLOYEE WHERE DNO = 30 AND JOB = 'SALESMAN';
...
SELECT ENAME, ENO, SALARY, DNO FROM EMPLOYEE WHERE ENAME LIKE '%E%' AND DNO = 30 AND SALARY NOT BETWEEN 1000 AND 2000;

-- 문제 22) 상여금이(COMMISSION) 없고 매니저가(MANAGER) 있고 직책이(JOB) MANAGER , CLERK 인 사원 중에서
-- 사원이름의 두번째 글자가 L 이 아닌 사원의 정보를 출력하는 SQL문을 작성하세요

SELECT * FROM EMPLOYEE WHERE COMMISSION IS NULL AND MANAGER IS NOT NULL AND JOB IN ('MANAGER', 'CLERK') AND
ENAME NOT LIKE '_L%';

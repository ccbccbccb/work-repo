...
FROM EMPLOYEE
GROUP BY JOB HAVING ROUND(AVG(SALARY),1) = (SELECT MIN(ROUND(AVG(SALARY),1))
                FROM EMPLOYEE
                GROUP BY JOB);
...
SELECT JOB, AVG(SALARY) AS 평균급여 FROM EMPLOYEE
...
SELECT JOB,ENO,ENAME,SALARY,EMP.DNO,DNAME FROM EMPLOYEE EMP, DEPARTMENT DEP WHERE EMP.DNO = DEP.DNO
...
SELECT ENO,ENAME,JOB,EMP.DNO,DNAME,LOC FROM EMPLOYEE EMP, DEPARTMENT DEP WHERE EMP.DNO = DEP.DNO
AND JOB IN (SELECT JOB FROM EMPLOYEE WHERE DNO = '10')

...
SELECT EMP.ENO, DEP.DNAME FROM EMPLOYEE EMP, DEPARTMENT DEP 
...
SELECT EMP.ENO, EMP.JOB, DEP.LOC FROM EMPLOYEE EMP, DEPARTMENT DEP
WHERE EMP.DNO = DEP.DNO AND EMP.DNO = 10;

-- 4) 커미션을(COMMISSION) 받는 모든사원의 이름(ENAME),
--       부서이름(DNAME), 지역명(LOC)을 출력하세요.
SELECT EMP.ENAME, DEP.DNAME, DEP.LOC FROM EMPLOYEE EMP, DEPARTMENT DEP
WHERE EMP.DNO = DEP.DNO AND COMMISSION IS NOT NULL;

-- 5) = 조인과 Like 검색(와일드카드(%))를 사용하여 
--     이름에 A가 포함된 모든 사원의 이름과(ENAME)
--    부서명을(DNAME) 출력하시오.

SELECT EMP.ENAME, DEP.DNAME FROM EMPLOYEE EMP, DEPARTMENT DEP
WHERE EMP.DNO = DEP.DNO AND ENAME LIKE '%A%';
...
SELECT EMP.ENAME, EMP.JOB, DEP.DNO, DEP.DNAME FROM EMPLOYEE EMP, DEPARTMENT DEP
WHERE EMP.DNO = DEP.DNO AND LOC = 'NEW YORK';
...
SELECT DEP.DNAME AS 부서명, EMP.SALARY AS 급여, COUNT(*) AS 사원수 FROM EMPLOYEE EMP, DEPARTMENT DEP
WHERE EMP.DNO = DEP.DNO GROUP BY DEP.DNAME, EMP.SALARY;
...
SELECT DEP.DNAME AS 부서명, EMP.JOB AS 담당업무 FROM EMPLOYEE EMP, DEPARTMENT DEP
WHERE EMP.DNO = DEP.DNO GROUP BY DEP.DNAME , EMP.JOB HAVING SUM(SALARY) >= 5000;

-- 10) 급여가(SALARY) 2000 초과인 사원들의 
--  부서번호(DNO), 부서명(DNAME), 사원번호(ENO), 사원명(ENAME), 급여를 출력하세요

SELECT DEP.DNO AS 부서번호, DEP.DNAME AS 부서명, EMP.ENO AS 사원번호, EMP.ENAME AS 사원명
FROM EMPLOYEE EMP, DEPARTMENT DEP
WHERE EMP.DNO = DEP.DNO AND SALARY > 2000;

-- 11) 각 부서별(DNO) 평균 급여(SALARY), 최대급여, 최소급여, 사원수를 출력해 보세요

SELECT DEP.DNO AS 부서번호, MAX(SALARY) AS 최대급여, MIN(SALARY) AS 최소급여, COUNT(*) AS 사원명
FROM EMPLOYEE EMP, DEPARTMENT DEP
WHERE EMP.DNO = DEP.DNO GROUP BY DEP.DNO;
...
FROM EMPLOYEE EMP
     ,DEPARTMENT DEP
WHERE EMP.DNO(+) = DEP.DNO; ORDER BY EMP.DNO ASC, EMP.ENAME ASC;   
